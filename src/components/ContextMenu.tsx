...
export function ContextMenu() {
  ...
  useLayoutEffect(() => {
    if (!contextMenu || !trapRef.current) { setAdjusted(null); return }
  ...
  return (
    ...
    >
      {entry.isDir ? (
        <button role="menuitem" className={styles.item} onClick={() => { navigate(entry.path); closeContextMenu() }}>
          <FolderOpen size={16} /> Открыть
        </button>
      ) : (
        <button role="menuitem" className={styles.item} onClick={() => { openViewer(entry); closeContextMenu() }}>
          <Eye size={16} /> Просмотр
        </button>
      )}
      <button role="menuitem" className={styles.item} onClick={() => { startRename(entry.path); closeContextMenu() }}>
        <Pencil size={16} /> Переименовать
      </button>
      <button role="menuitem" className={styles.item} onClick={() => { copyToClipboard([entry]); closeContextMenu() }}>
        <Copy size={16} /> Копировать
      </button>
      <button role="menuitem" className={styles.item} onClick={() => { cutToClipboard([entry]); closeContextMenu() }}>
