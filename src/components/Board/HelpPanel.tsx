  ...
  return (
    <div className={styles.wrap}>
      ...
      <Section title="Шаблоны, штучки, анимации">
        ...
        <Row left="Выравнивание">
          Выдели две карточки и больше — сверху появится палитра: по левому краю, по центру,
          по правому, по верху, по середине, по низу. С тремя и больше включаются «равные
          промежутки» (крайние остаются на месте), а «В ряд» и «В столбец» сдвигают всё
          вплотную с одинаковым зазором.
        </Row>
        <Row left="Дублирование">
          <Keys>Alt</Keys> + перетаскивание оставляет копию на месте и тащит новую — самый
          быстрый способ разложить ряд одинаковых карточек. Работает и на нескольких сразу.
        </Row>
        <Row left="Точный сдвиг">
          Стрелки двигают выделенное на 1px, <Keys>Shift</Keys>+стрелки — на 10px.
        </Row>
        <Row left="Блокировка">
          <Keys>Ctrl+2</Keys> запирает выделенное: такую карточку не сдвинуть и не растянуть,
          в углу появляется замок. <Keys>Ctrl+Alt+2</Keys> снимает со всех. Для фона и рамок,
          которые постоянно хватаешь по ошибке.
        </Row>
        <Row left="Карандаш">
          Кнопка с карандашом в нижней панели — и рисуй прямо по холсту мышью или пальцем.
          Сверху появится палитра: шесть цветов, четыре толщины, «Готово». Карандаш остаётся
          в руке, пока не уберёшь его (<Keys>Esc</Keys> или «Готово»), а все штрихи, сделанные
          подряд, складываются в один рисунок — букву из трёх линий никто не считает тремя
          объектами. Рисунок потом двигается, тянется за углы (линии масштабируются вместе с
          рамкой) и живёт как обычная карточка.
        </Row>
        <Row left="Рисование фигур">
          Кнопка «Фигура» в нижней панели → выбери форму → растяни рамку прямо на холсте:
          фигура появится того размера, который нарисовал. <Keys>Shift</Keys> во время
          растягивания держит квадрат (или круг), <Keys>Esc</Keys> отменяет режим. Кнопка
          выбранной формы подсвечена, пока режим включён; повторный клик по ней выключает.
        </Row>
        <Row left="Фигура как контейнер">
          В правом меню фигуры — «Держать содержимое в себе». После этого фигура ведёт себя
          как контейнер: тащишь её — едет со всем, что внутри, и появляется «Разложить
          содержимое». По умолчанию выключено: фигуры часто рисуют позади карточек просто
          для акцента, и таскать их за собой они не должны.
        </Row>
        <Row left="Контейнер">
