  ...
  return (
    <div className={styles.wrap}>
      ...
      <Section title="Шаблоны, штучки, анимации">
        ...
        <Row left="Контейнер">
          Выдели карточки и нажми <Keys>Ctrl+G</Keys> (или кнопку с рамкой в нижней панели) —
          вокруг них появится подписанный контейнер. Тащишь контейнер — едет со всем
          содержимым. Правое меню контейнера: «Разложить содержимое» выстраивает карточки
          ровной сеткой с одинаковыми отступами, «Убрать контейнер» снимает рамку и оставляет
          карточки на месте (<Keys>Ctrl+Shift+G</Keys>). Без выделения кнопка создаёт пустой
          контейнер — обведи им что угодно потом.
          Принадлежность определяется по тому, что видно: карточка внутри контейнера, если
          внутри её середина. Ничего не «привязывается» и не отваливается втихую.
        </Row>
        <Row left="Направляющие">
          При перетаскивании и растягивании карточка сама встаёт вровень с соседями: как только
          край или центр подходит близко к чужому краю или центру, появляется бирюзовая линия и
          лёгкое притяжение. Линии показываются только рядом с другими карточками и гаснут, как
          отпустишь. Притяжение к соседу важнее сетки.
        </Row>
